...
class OCRService {
  ...
  async extractFromPDF(filePath) {
    try {
      ...
      // console.log("Data buffer", dataBuffer.slice(0, 100));
      // console.log("Data buffer type", dataBuffer.type);
      ...
      if (!text || text.length < 10) {
        throw new Error(
          "PDF appears to be empty or contains only images. OCR for image-based PDFs coming soon.",
        );
      }
  ...
  async extractWithClaudeVision(imageUrl) {
    try {
      ...
      throw new Error(
        `Failed to extract text with Claude Vision: ${error.message}`,
      );
    }
  }
}
